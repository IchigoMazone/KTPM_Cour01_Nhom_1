...
class AccountService:
    ...
    def login_or_register_google(self, email: str, full_name: str):
        cursor = self.connect.cursor()
        try:
            repository = AccountRepository(cursor)
            # 1. Tìm xem email đã tồn tại trong user_profiles chưa
            account = repository.get_account_by_email(email)
            
            if not account:
                # 2. Nếu chưa có, tiến hành tạo mới tài khoản
                base_username = email.split("@")[0].lower()
                import re
                base_username = re.sub(r'[^a-z0-9_]', '', base_username)
                if len(base_username) < 8:
                    base_username = base_username.ljust(8, '_')
                elif len(base_username) > 12:
                ...
                # Sinh mật khẩu ngẫu nhiên độ bảo mật cao
                import uuid
                password_hash = bcrypt.hashpw(str(uuid.uuid4()).encode('utf-8'), bcrypt.gensalt()).decode('utf-8')
                
                # Tạo tài khoản user và profile tương ứng
                user_id = repository.create_account(username, password_hash, role="user")
                repository.create_profile(user_id, full_name, email)
                self.connect.commit()
                
                account = (user_id, username, "user")
            
            # 3. Ký và trả về local JWT token
            access_token = create_access_token({
                "user_id": str(account[0]),
                "username": account[1],
                "role": account[2]
            ...
            return {
                "success": False,
                "message": f"Có lỗi xảy ra khi xử lý đăng nhập Google: {str(e)}"
            }
        finally:
            cursor.close()
